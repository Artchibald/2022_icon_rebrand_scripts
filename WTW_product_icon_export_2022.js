...
if (sourceDoc.artboards[0].name = "Core") {
    var CSTasks_1 = (function () {
        ...
        tasks.getArtboardCorner = function (artboard) {
            var corner = [sourceDoc.artboards[0].artboardRect[0], artboard.artboardRect[1]];
            return corner;
        };
    ...
    function main() {
        ...
        if (sourceDoc.artboards.length == 1 &&
            ...
            sourceDoc.artboards.add(CSTasks_1.newRect(firstRect[2] + gutter, firstRect[1], 2400, 256));
        }
        //if the masthead artboard is present, check if rebuilding or just exporting
            ...
            if (rebuild)
                CSTasks_1.clearArtboard(sourceDoc, 1);
            else
                return;
        }
        //otherwise abort
        ...
        var sel = CSTasks_1.selectContentsOnArtboard(sourceDoc, 0);
        if (sel.length == 0) {
            //if nothing is in the artboard
            alert("Please try again with artwork on the main 256x256 artboard.");
            return;
        }
        var colors = CSTasks_1.initializeColors(RGBColorElements, CMYKColorElements); //initialize the colors from the brand palette
        var iconGroup = CSTasks_1.createGroup(sourceDoc, sel); //group the selection (easier to work with)
        var iconOffset = CSTasks_1.getOffset(iconGroup.position, CSTasks_1.getArtboardCorner(sourceDoc.artboards[0]));
        /********************************
        ...
        CSTasks_1.translateObjectTo(mast, mastPos);
        ...
        CSTasks_1.setFont(textRef, desiredFont);
        ...
        var vOffset = CSTasks_1.getOffset(textRef.anchor, [0, bottomEdge]);
        ...
        var hOffset = CSTasks_1.getOffset(textGroup.position, [rightEdge, 0]);
        ...
        var newWidth = textGroup.position[0] +
            ...
            leftMargin;
        var resizedRect = CSTasks_1.newRect(sourceDoc.artboards[1].artboardRect[0], -sourceDoc.artboards[1].artboardRect[1], newWidth, 256);
        sourceDoc.artboards[1].artboardRect = resizedRect;
        //get the text offset for exporting
        var mastTextOffset = CSTasks_1.getOffset(textGroup.position, CSTasks_1.getArtboardCorner(sourceDoc.artboards[1]));
        /*********************************************************************
        ...
        var rgbDoc = CSTasks_1.duplicateArtboardInNewDoc(sourceDoc, 0, DocumentColorSpace.RGB);
        ...
        CSTasks_1.translateObjectTo(rgbGroup, rgbLoc);
        CSTasks_1.ungroupOnce(rgbGroup);
        ...
        for (var i_7 = 0; i_7 < PNGSizes.length; i_7++) {
            ...
            CSTasks_1.scaleAndExportPNG(rgbDoc, destFile_1, startWidth, PNGSizes[i_7]);
        }
        ...
        var colorIndex = CSTasks_1.indexRGBColors(rgbDoc.pathItems, colors);
        //convert violet to white and save as EPS
        CSTasks_1.convertColorRGB(rgbDoc.pathItems, colors[violetIndex][0], colors[whiteIndex][0]);
        ...
        for (var i_8 = 0; i_8 < PNGSizes.length; i_8++) {
            ...
            CSTasks_1.scaleAndExportPNG(rgbDoc, destFile_2, startWidth, PNGSizes[i_8]);
        }
        //convert to inactive color (WTW Icon grey at 50% opacity) and save as EPS
        CSTasks_1.convertAll(rgbDoc.pathItems, colors[grayIndex][0], 50);
        ...
        for (var i_9 = 0; i_9 < PNGSizes.length; i_9++) {
            ...
            CSTasks_1.scaleAndExportPNG(rgbDoc, destFile_3, startWidth, PNGSizes[i_9]);
        }
        //close and clean up
        ...
        var cmykDoc = CSTasks_1.duplicateArtboardInNewDoc(sourceDoc, 0, DocumentColorSpace.CMYK);
        ...
        CSTasks_1.translateObjectTo(cmykGroup, cmykLoc);
        CSTasks_1.ungroupOnce(cmykGroup);
        CSTasks_1.convertToCMYK(cmykDoc, cmykDoc.pathItems, colors, colorIndex);
        //save EPS into the export folder
        ...
        //convert violet to white and save as EPS
        CSTasks_1.convertColorCMYK(cmykDoc.pathItems, colors[violetIndex][1], colors[whiteIndex][1]);
        ...
        var mastDoc = CSTasks_1.duplicateArtboardInNewDoc(sourceDoc, 1, DocumentColorSpace.RGB);
        ...
        CSTasks_1.translateObjectTo(mastGroup, mastLoc);
        CSTasks_1.ungroupOnce(mastGroup);
        ...
        CSTasks_1.translateObjectTo(mastText, mastTextLoc);
        ...
        /************
          Final cleanup
          ************/
        CSTasks_1.ungroupOnce(iconGroup);
        CSTasks_1.ungroupOnce(mast);
